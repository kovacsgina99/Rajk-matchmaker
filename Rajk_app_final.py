...
with open("data_streamlit.json", "r", encoding="utf-8") as f:
    ...
    names = data["names"]
    match_percentages = {
        tuple(k.split("_")): v for k, v in data["match_percentages"].items()
    }

with open("data2_v2_streamlit.json", "r", encoding="utf-8") as f:
    top_5_matches = json.load(f)["top_5_matches"]
...
def rajk():
    ...
    if st.button("Mehet"):
        if (f1_list_names1, f1_list_names2) in match_percentages:
            ...
            formatted_percentage = (
                f"{round(percentage * 100, 2):.2f}%"  # eredmény formázása
            )
    ...
    relationship_type = st.radio(
        ...
        ("Hetero", "Meleg", "Mindkettőre nyitott vagyok"),
    )
        ...
        if f2_list_names in top_5_matches:
            ...
            if top_5_final_matches:
                ...
                for name, match_percent, hetero_meleg in top_5_final_matches:
                    formatted_percentage = (
                        f"{round(match_percent * 100, 2):.2f}%"  # eredmény formázása
                    )
